  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {loading ? 'Loading User Profile' : fetchError ? 'Error Loading Profile' : `User Profile - ${detailedUser?.full_name}`}
          </DialogTitle>
        ...
        {fetchError && !loading && (
          <Card className="p-4 border-red-200 bg-red-50">
            ...
            <Button onClick={fetchDetailedUser} className="w-full bg-blue-600 hover:bg-blue-700">
              Retry
            </Button>
          </Card>
        )}

        {detailedUser && !loading && !fetchError && (
